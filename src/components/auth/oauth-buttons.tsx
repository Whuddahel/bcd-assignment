...
const PROVIDERS: { id: OAuthProvider; label: string; icon: React.ElementType }[] = [
...
export function OAuthButtons({ next }: { next?: string }) {
  ...
  async function handleClick(provider: OAuthProvider) {
    setPending(provider)
    try {
      // On success the browser navigates away, so `pending` stays set.
    ...
    <div className="space-y-2.5">
      {PROVIDERS.map(({ id, label, icon: Icon }) => (
        <button
          ...
          className="flex h-11 w-full items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/5 text-sm font-medium text-foreground transition-all hover:border-white/20 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-60"
        >
