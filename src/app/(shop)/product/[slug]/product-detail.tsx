            ...
            <div className={cn("relative aspect-square overflow-hidden rounded-3xl bg-gradient-to-br", product.gradient)}>
              ...
              <div className="absolute right-4 top-4 z-10 flex flex-col gap-2">
                <button
                  ...
                  disabled={pending}
                  className={cn(
                    "flex h-10 w-10 items-center justify-center rounded-full backdrop-blur-sm transition-all disabled:opacity-60",
            ...
            {product.images.length > 1 && (
              <div className="mt-3 flex gap-2">
                {product.images.slice(0, 4).map((src, i) => (
                  <div key={i} className="relative h-16 w-16 overflow-hidden rounded-xl border border-white/10">
                    <Image src={src} alt={`${product.title} ${i + 1}`} fill sizes="64px" className="object-cover" />
                  </div>
                ))}
              </div>
            )}
