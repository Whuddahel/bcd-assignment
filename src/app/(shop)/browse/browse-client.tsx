...
import type { ProductVM, CategoryVM } from "@/lib/data/types"
...
export function BrowseClient({
  ...
  const filtered = useMemo(() => {
    ...
    return items
  }, [products, query, category, condition, priceIdx, sort])
        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filtered.map((p, i) => (
                <ProductCard key={p.id} product={p} index={i} wishlistDefault={wishlisted.has(p.id)} />
              ))}
            </div>
          </>
        )}
